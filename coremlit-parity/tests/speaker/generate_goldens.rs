...
//!
//! Gated on the `speaker-oracle` feature (dia's own ort inference path) so the
...
//! (whisperkit's convention, see `coremlit/tests/whisper/parity_jfk.rs`):
...
//!   `speaker-oracle` feature enables), no file needed.
...
/// sibling `diarization/models/wespeaker_resnet34_lm.onnx` (beside the
/// checkout, resolved through `common::checkout_parent()` — a sibling-checkout
/// convention; the `dia` dep itself is a git dependency pinned to an exact
/// rev, not a path dep).
...
#[test]
#[ignore = "rewrites committed goldens; set UPDATE_GOLDEN=1 + `speaker-oracle` feature + wespeaker ONNX"]
fn generate_goldens() {
