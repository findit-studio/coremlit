...
/// checkout (override root via `DIA_PARITY_FIXTURES`). Same sibling-checkout
/// convention as the co-dev `[patch]` for the git-pinned `dia` dep and
/// `generate_goldens.rs`'s wespeaker ONNX resolution — anchored on
/// `common::checkout_parent()`, never on a hop count.
...
fn equal_delta_der_hides_disjoint_arm_errors() {
  ...
  // CpuOnly is silent over [10, 20); All is silent over [50, 60). Disjoint.
  let cpu = vec![
    Seg {
      start: 0.0,
      end: 10.0,
  ...
  let all_vs_ref = der_std(&reference, &all);
  assert_eq!(
...
/// invariant) stands; what changed here is that it is now ASSERTED rather than
/// printed, and asserted for the argmax source too — which previously ran its
...
/// `coremlit-parity/tests/...`, which does not exist — and the failure
