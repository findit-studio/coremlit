...
| [`speakerkit`](crates/speakerkit) | Speaker diarization on CoreML: runs pyannote's `segmentation-3.0` net and the WeSpeaker embedder on the Neural Engine (via `coremlit`), assembles the `dia`-shaped tensors (`Extraction`), then clusters them into speaker-labelled RTTM spans with [`diaric`](https://github.com/findit-studio/diaric)'s ONNX-free Rust clustering core — offline VBx/PLDA (`Extraction::diarize` / `diarize_with`) and the online greedy centroid matcher (`Extraction::diarize_online`). Multi-source — the FluidAudio (default) and Argmax model layouts behind one `Source` API — with host-side powerset/mask/window decode ported from `dia`. `Extraction::into_offline_input` bridges straight into `diaric::offline::diarize_offline`, and the runtime graph pulls no `ort`/ONNX in any configuration. DER-parity-gated end to end against pyannote-output references, scored against the `dia` (`diarization`) ort oracle behind the test-only `dia-oracle` feature. |
