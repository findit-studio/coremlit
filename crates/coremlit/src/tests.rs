...
/// The house rule this module enforces over every `.rs` file in `crates/`
...
mod variant_shape {
  ...
  /// `<workspace>/crates`, the tree the rule holds over.
  ...
  /// Every `.rs` file under `dir`, skipping any `target/` (build artifacts are
...
/// **The sweep's finish line.** No enum variant anywhere in `crates/` is
...
#[test]
fn no_enum_in_crates_has_a_struct_shaped_or_multi_field_variant() {
  use variant_shape::{crates_root, rust_files, variants_in};

  let root = crates_root();
  assert!(
    root.is_dir(),
    "the workspace's crates/ directory must be reachable at {}",
    root.display()
  );
