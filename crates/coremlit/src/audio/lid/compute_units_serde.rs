...
//! through its existing `as_str`/`FromStr` — the same shape the
//! ced/granite/clap modules' private compute-units bridges use. Used by
//! [`crate::audio::lid::IdentifierOptions`] via
//! `serde(with = "crate::audio::lid::compute_units_serde")`.
