...
/// [`AudioEncoder`] per worker over a `Send` [`crate::Model`]
...
impl AudioEncoder {
  ...
  /// [`AggregatePolicy`](crate::embeddings::clap::aggregate::AggregatePolicy), score each window
  /// ([`crate::embeddings::clap::score::score_windows`]), or both.
  ///
  /// Runs sequentially: [`crate::Model`] is `!Sync`, so windows share one
