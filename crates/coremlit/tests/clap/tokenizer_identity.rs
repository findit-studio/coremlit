...
//!
//! 1. **This file** pins the PUBLIC [`coremlit::embeddings::clap::BUNDLED_TOKENIZER`] const's
...
fn bundled_tokenizer_matches_pinned_sha256() {
  ...
  let sha: String = Sha256::digest(coremlit::embeddings::clap::BUNDLED_TOKENIZER)
    .iter()
    .map(|b| format!("{b:02x}"))
    .collect();
  assert_eq!(
    sha, PINNED_TOKENIZER_SHA256,
    "coremlit::embeddings::clap::BUNDLED_TOKENIZER diverged from the pinned Xenova tokenizer textclap uses"
  );
  // Non-vacuity: the bundled artifact is the real ~2 MB tokenizer, not a stub.
  assert!(
    coremlit::embeddings::clap::BUNDLED_TOKENIZER.len() > 1_000_000,
    "bundled tokenizer is implausibly small ({} bytes)",
    coremlit::embeddings::clap::BUNDLED_TOKENIZER.len()
  );
}
