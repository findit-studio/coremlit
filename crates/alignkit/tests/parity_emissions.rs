...
use alignkit::encode::{ENCODER_WINDOW_SAMPLES, Encoder, EncoderOptions};
...
/// Per-frame comparison statistics between the CoreML and asry-ONNX
/// emissions matrices, over the frames both sides share
/// (`min(coreml.t(), asry.t())`).
struct ParityStats {
...
/// Computes [`ParityStats`] between `coreml` and `asry_reference`.
///
/// # Panics
/// If the two sides' vocab dimension `V` disagrees — that would mean the
/// two models are not directly comparable at all (a tokenizer/model
/// pairing bug), not a numeric parity question this stat block can
/// usefully summarize.
fn compare(coreml: &LogProbsTV, asry_reference: &AsryReference) -> ParityStats {
  assert_eq!(
    coreml.v(),
    asry_reference.v,
    "vocab dim mismatch between CoreML ({}) and asry-ONNX ({}) emissions — models are not \
     directly comparable",
    coreml.v(),
    asry_reference.v
  );
  ...
  for t in 0..frames_compared {
    ...
    for v in 0..vocab {
      let a = coreml.at(t, v);
      let b = asry_reference.data[t * vocab + v];
      let diff = (a - b).abs();
...
/// just the pass/fail outcome.
fn report(label: &str, stats: &ParityStats) {
  eprintln!(
    "[gate1] {label}: frames_compared={} max_abs_diff={:.6} (at frame {}) \
     argmax_agreement={:.4}% ({}/{}) disagreements(frame,coreml,asry)={:?}",
...
/// (and its accepted skip-looks-like-pass hazard), and the Task B3 report
/// for the measured-numbers gap this leaves.
#[test]
...
fn emissions_match_asry_ort_reference() {
  ...
  let stats_full = compare(&coreml_full, &asry_full);
  report("ted_60.wav (full 960,000-sample window)", &stats_full);
  ...
  let stats_short = compare(&coreml_short, &asry_short);
  report("48,000-sample slice (truncation-rule case)", &stats_short);
