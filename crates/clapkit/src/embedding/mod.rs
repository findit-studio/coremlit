...
/// Pinned from the converted graphs' output contract (`tests/model_io.rs` /
...
/// # let a = clapkit::Embedding::from_slice_normalizing(&s).unwrap();
/// # let b = a.clone();
/// let _ = a == b;
/// ```
#[derive(Clone)]
...
impl Embedding {
  ...
  /// Crate-internal constructor for encoders whose upstream guard already
