...
/// graph); placement is still characterized, not asserted (`tests/placement.rs`).
...
pub struct TextEncoderOptions {
  #[cfg_attr(
    feature = "serde",
    serde(default = "default_text_compute", with = "crate::compute_units_serde")
  )]
...
impl TextEncoder {
  ...
  /// tokenizer ([`crate::BUNDLED_TOKENIZER`]) and [`TextEncoderOptions::new`].
  ///
  /// # Errors
  /// As [`Self::from_files`].
  ...
  pub fn from_bundled_tokenizer(
    ...
    let tokenizer =
      Tokenizer::from_bytes(crate::BUNDLED_TOKENIZER).map_err(Error::TokenizerLoad)?;
  ...
  /// sequence that is identity-comparable to textclap (`tests/tokenizer_identity.rs`).
  ///
  /// # Errors
  ...
  /// diverges from `[1, `[`EMBEDDING_DIM`]`]`; [`Error::NonFiniteEmbedding`] /
