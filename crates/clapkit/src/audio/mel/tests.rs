...
/// Read `tests/fixtures/mel/sample.wav` into normalized f32, asserting its 48 kHz
...
fn filterbank_rows_match_librosa() {
  ...
  for &row_idx in &[0usize, 10, 32] {
    let expected = read_npy_f32_shaped(
      &format!("tests/fixtures/mel/filterbank_row_{row_idx}.npy"),
      &[N_FREQ as u64],
    );
...
fn extract_into_matches_textclap_golden_mel() {
  ...
  let golden = read_npy_f32_shaped(
    "tests/fixtures/mel/golden_mel.npy",
