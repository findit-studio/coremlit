...
fn spellings_roster_is_the_known_set() {
  // Pins the current roster concretely (the single variant so far). Adding a
  // variant to the macro table updates this AND the loops above together.
  assert_eq!(CLUSTER_BACKEND_SPELLINGS, &["offline"]);
}
...
fn unknown_name_is_opaque_error() {
  // `online` is not (yet) a variant; empty and arbitrary names fail too.
  assert!("online".parse::<ClusterBackend>().is_err());
...
fn serde_offline_partial_payload_defaults_other_knobs() {
  ...
  let ClusterBackend::Offline(o) = b;
