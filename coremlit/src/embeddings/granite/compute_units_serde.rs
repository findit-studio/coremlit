...
//! through its existing `as_str`/`FromStr` — the same shape the CLAP module's
//! private compute-units bridge uses. Used by
//! [`crate::embeddings::granite::TextEmbedderOptions`] via
//! `serde(with = "crate::embeddings::granite::compute_units_serde")`.
