...
//! through its existing `as_str`/`FromStr` — the same shape the `siglip`,
//! `granite` and `clap` modules' private compute-units bridges use. Used by
//! [`crate::embeddings::face::FaceEmbedderOptions`] via
//! `serde(with = "crate::embeddings::face::compute_units_serde")`.
