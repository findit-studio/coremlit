...
fn gate(recorded: Option<CharacterizedHost>) -> BandGate {
  BandGate::open_with(
    ...
    recorded,
    running_host(),
    common::recharacterize_command("siglip_placement", "coremlit/tests/siglip/placement.rs"),
  )
}
...
fn the_unrecorded_banner_explains_itself() {
  ...
  for needle in [
    ...
    "coremlit/tests/siglip/placement.rs",
  ] {
    assert!(
...
fn measured_constants_are_routed_through_the_gate() {
  let cases: [(&str, &[&str], &[&str]); 2] = [
    (
      "coremlit/tests/siglip/parity_embed.rs",
      &["MEASURED_FLOOR"],
      &["PARITY_FLOOR"],
    ),
    (
