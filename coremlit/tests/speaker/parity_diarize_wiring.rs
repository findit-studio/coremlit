...
//! [`coremlit::audio::speaker::extract::Extraction::diarize`], is ONE code path with the
...
//!
//! ```text
//! SPEAKERKIT_TEST_MODELS=… cargo test -p coremlit --test speaker_parity_diarize_wiring -- --ignored --nocapture
//! ```
//!
//! # Clip selection
//!
...
/// via `DIA_PARITY_FIXTURES`) — the same sibling-checkout convention as the
/// crate's `dia` path dependency, anchored on `common::checkout_parent()`
