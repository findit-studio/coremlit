...
// Relative to THIS package directory (`coremlit/tests/speaker/swift`): five
// hops reach the directory holding the checkout, where the sibling clone
// lives. `regen_goldens.sh` normally overrides this with `ARGMAX_SWIFT_SRC`,
// which it resolves by SEARCHING for the workspace root; this literal is the
// bare-`swift build` fallback, and SwiftPM errors loudly on a path that does
// not resolve to a package rather than degrading silently.
