...
- `coremlit/src/audio/whisper/decode/filter/mod.rs` — `bnns_mass_rule_scalars`
  (H1: BNNS f16 timestamp-mass rule replication)
- `coremlit/src/audio/whisper/decode/sampler/mod.rs` — `argmax`
  (H2: first-index, NaN-skipping tie-break)
- `coremlit/src/audio/whisper/segment/mod.rs` — `coreml_f16_row_pitch` /
...
a concrete oracle capture, following the `coremlit/tests/speaker/swift/`
...
`coremlit/tests/whisper/fixtures/golden/jfk_tiny_words_golden.json` is a
...
path and writes nothing into it (the `coremlit/tests/speaker/swift/`
...
`coremlit/tests/whisper/swift/regen_goldens.sh` drives the same oracle
