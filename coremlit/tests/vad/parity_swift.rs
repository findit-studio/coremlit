...
const VAD_REGEN_SCRIPT: &str = "coremlit/tests/vad/swift/regen_goldens.sh";
...
const GENERATOR: &str = "coremlit/tests/vad/swift/Tests/VadTraceDump/DumpVadTraces.swift";
...
fn load_swift_golden(name: &str) -> SwiftGolden {
  ...
  let bytes = std::fs::read(&path).unwrap_or_else(|e| {
    panic!(
      "read swift golden {}: {e}\n  regenerate: coremlit/tests/vad/swift/regen_goldens.sh",
      path.display()
    )
  });
...
fn well_formed() -> serde_json::Value {
  serde_json::json!({
    "fixture": "wf",
    "generator": "coremlit/tests/vad/swift/Tests/VadTraceDump/DumpVadTraces.swift",
