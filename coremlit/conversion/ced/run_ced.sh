...
# `MODELS_LOCK`, a file that exists only at this repository's root. A `../` hop
# count encodes this script's depth in the tree and fails silently when that
# depth changes, staging the converted models into some other directory that
# happens to exist.
REPO="$HERE"
...
OUT_ROOT="${CED_MODELS_OUT:-$REPO/Models/ced}"
