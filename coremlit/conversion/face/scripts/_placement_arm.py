...
Why a subprocess per arm rather than a loop — the same two reasons the ReDimNet recipe
gives, and they apply unchanged here:
  ...
  * **cold load.** CoreML caches compiled ANE programs per process, so a second load in one
    process is not a load. A fresh process per arm is the only way each arm's load time
    means the same thing.

Usage: ``python _placement_arm.py <arm> <mlmodelc> <inputs.npy> <repeats>``. One line on
stdout prefixed ``@@ARM@@`` carries the result; everything else on stdout is runtime noise.
"""
import json
import sys
import time
...
from _arcface_common import INPUT_NAME, OUTPUT_NAME
...
def main(arm, path, inputs_npy, repeats):
    xs = np.load(inputs_npy)                       # [n_faces, 1, 3, 112, 112] f32
    result = {"arm": arm, "load_ms": None, "first_predict_ms": None, "warm_predict_ms": None,
              "warm_predict_p10_ms": None, "embeddings": None, "error": None}
    try:
        ...
        result["warm_predict_p10_ms"] = float(np.percentile(times, 10))
