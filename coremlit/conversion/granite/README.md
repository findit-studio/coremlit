...
export GRANITE_GOLDENS="$PWD/coremlit/tests/granite/fixtures/goldens"
...
bash coremlit/conversion/granite/run_granite.sh
```
...
| `tests/test_toolchain_binding.py` | the producer/consumer toolchain guard, negative cases included; stdlib `unittest`, no model or venv needed (`python3 -m unittest discover -s coremlit/conversion/granite/tests`) |
